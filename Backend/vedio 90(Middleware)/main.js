...
// 3. The next middleware function in the application’s request-response cycle (called next).
// ---------------
...
//3. End the request-response cycle (e.g., send a response).

//4. Call the next middleware in the stack using next().
// --------------

// Types of Middleware: ***IMP
//1. Application-level Middleware: Bound to the app using app.use() or app.METHOD().

// 2.Router-level Middleware: Bound to a specific router using router.use().

//3. Error-handling Middleware: Handles errors (takes 4 arguments: err, req, res, next).

//4. Built-in Middleware: Provided by Express (e.g., express.json()).

//5. Third-party Middleware: Added via npm (e.g., morgan, cors).
...
app.use(express.static("public")) //1.  app.use (middleware) attract the request and ask the "/hari.txt"(written by user in the url bar) kya app public folder me ho . toh answer yes aata hai // 2. to run this move to the cd 'vedio 90'
